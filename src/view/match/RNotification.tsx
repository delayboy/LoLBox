...
export default function ({ notice, onCloseInput }: { notice: NoticeTypes, onCloseInput: any }) {
  ...
  const mapMatch = (index:number) =>{
     ...
     <div key={index} className='flex-col flex'style={{paddingRight:'1px',paddingLeft:'1px'}}>
       <div className='flex-row flex' style={{backgroundColor:backgroundColor,width:'95px'}}>
        ...
        <span style={{width:'95px',fontSize:'10px',color:color,backgroundColor:backgroundColor}}>{` ${value.score}分 ${value.winRate}%`}</span>
        <span style={{width:'95px',fontSize:'10px',color:color,backgroundColor:backgroundColor}}>{` ${value.horse}`}</span>
        <span style={{width:'95px',fontSize:'10px',color:color,backgroundColor:backgroundColor}}>{` id:${value.gameName}`}</span>
        <span style={{width:'95px',fontSize:'10px',color:color,backgroundColor:backgroundColor}}>{` (${inBlackList})`}</span>
         {matchList.map((match,i)=>{
           ...
           <div  onClick={()=>{onClose();alterToSumId(value.summonerId);handleMatchList(matchList,i)}}  key={index*20+i} className='flex-row flex' style={{paddingBottom:'1px',paddingTop:'1px'}}>
            <Image className='rounded' boxSize='27px' src={getImgUrl(match.champId)}/>
            <Tag
             size={'sm'}

...
export default function ({ sumId, puuid, begIndex, endIndex, openSumDetailDrawer, matchMode }: MatchHistoryTypes) {
  ...
  useEffect(() => {
    const fetchMatchList = async () => {
      ...
      if(customDetail.matchList!==undefined){
        ...
        customDetail={};
      }else  handleMatchList(matchList, 'error');
    }
    ...
    const fetchSpecialMatchList = async () => {
      ...
      if(customDetail.matchList!==undefined){
        ...
        handleMatchList(customDetail.matchList, 'none',customDetail.matchIndex);
        customDetail={};
      }else  handleMatchList(matchList, 'none');
    }
    if (matchMode !== '0') {
  ...
  const handleMatchList = (matchList: MatchList[], noticeStr: string,startIndex=0) => {
    if(matchList.length<=startIndex)startIndex=0;
    setMatchIndex(startIndex)
    setMatchListProps(matchList)

    if (matchList.length > startIndex) {
      const gameId = matchList[startIndex].gameId
      setCurrentGameId(gameId)
      ...
      setCurrentGameId(noticeStr)
    }
  }
  const handleTypeIndex = (index:number)=>{
    setShowTypeIndex(index);
  }
  document.addEventListener("myCustomEvent", (msg: any) => {
    customDetail = msg.detail;
    handleMatchList(customDetail.matchList, 'none',customDetail.matchIndex);
  ...
  if (currentGameId === 'error') {
    ...
    return (<div className='p-3 bg-white h-full w-full boxShadow divContentCenter'>
      [ {queryGameType(Number(matchMode))} ] 当前页数下, 此召唤师的战绩为空</div>)
  } else if (currentGameId === 'loading') {
    return (<div className='p-3 bg-white h-full w-full boxShadow divContentCenter'>
      [ {queryGameType(Number(matchMode))} ] 正在查询,请耐心等待...</div>)
  }
    ...
    const matchDetail = currentQueueId === 1700
      ...
      : <MatchDetail key={matchIndex} typeIndex={showTypeIndex} typeIndexChange={handleTypeIndex} participantsInfo={participantsInfo} openDrawer={openSumDetailDrawer} />
    ...
    return (
      <div className='p-3 bg-white h-full w-full boxShadow'>
        <Grid
          ...
          gap="66px"
        >
          <GridItem style={{ width: '185px', height: '85vh', overflow: 'auto' }} colSpan={1}>
